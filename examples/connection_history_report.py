...
class ConnectionHistoryStatsReportApp(SCCApp):

    def add_options(self, parser):
        super(ConnectionHistoryStatsReportApp, self).add_options(parser)
        ...
        parser.add_option(
            '--traffic_type', dest='traffic_type', default='max',
            help='Type of traffic to query, either for max or total')

        parser.add_option('--device', dest='device', default=None,
                          help='Device ID')

    def validate_args(self):
        super(ConnectionHistoryStatsReportApp, self).validate_args()
...
if __name__ == '__main__':
    ConnectionHistoryStatsReportApp().run()
