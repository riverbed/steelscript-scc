...
class ThroughputPerApplStatsReportApp(SCCApp):

    def add_options(self, parser):
        super(ThroughputPerApplStatsReportApp, self).add_options(parser)
    ...
    def validate_args(self):
        super(ThroughputPerApplStatsReportApp, self).validate_args()

        if not self.options.devices:
            self.parser.error("Comma separated device IDs are required")

    def main(self):
        with ThroughputPerApplStatsReport(self.scc) as report:
...
if __name__ == '__main__':
    ThroughputPerApplStatsReportApp().run()
